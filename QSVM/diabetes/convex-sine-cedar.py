...
def kernel_matrix_rectangular(A, B,i):
 ...
 #   for i in range(len(A)):
    for j in range(len(B)):
        #if ( i < j):
        #res[0,j] = alpha*np.exp(- gamma*(2 - kernel(A[0], B[j]))**2) +(1-alpha)*np.tan(np.pi/(4+gamma*(2-kernel(A[0],B[j]))**2))
         #   else:
        # np.exp((-2 + kernel(A[0], B[j])))
        ke_call = kernel(A[0], B[j])
        res[0,j] = alpha*np.exp(- gamma*(2 - ke_call)**2 ) + (1-alpha)*np.sin(np.pi/(2+sigma*(2-ke_call)**2))
             #   res[i,j] = res[j,i]
    return list(res[0])
